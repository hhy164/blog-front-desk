...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
  ...
  const data = {
    ...
    [`rows|${query.limit | 10}`]: [{
      id: "@guid",
      title: "@ctitle(1,20)",
      description: "@cparagraph(1,10)", //描述生成1-10个句子
      category: {
        // 所属分类
        "id|1-10": 0,
        name: "分类@id",
      ...
      "thumb|1": [
        "@image(300x250, @color, #f40, @natural)",
        null
      ], //"缩略图地址"
      createDate: `@date('T')` //博客的创建时间(发布日期)---时间戳
    }]
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
  ...
  data: {
    ...
    "thumb|1": [Mock.Random.image("300x250", "#000", "#f40", "RandomImage"), null], //"缩略图地址"
